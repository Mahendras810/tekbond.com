 ...
 * @endcode
 */
$settings['hash_salt'] = 'SYExVQ0Z3s25zzmkoLovcGiH2oSOkASCBAiQxqAa8qBcgqv8TWTdPvmyUNizRZETL1UXR-7AHA';
...
if (file_exists(__DIR__ . '/local.settings.php')) {
  include __DIR__ . '/local.settings.php';  // Local environment configuration
} elseif (file_exists(__DIR__ . '/staging.settings.php')) {
  include __DIR__ . '/staging.settings.php';  // Staging environment configuration
} elseif (file_exists(__DIR__ . '/prod.settings.php')) {
  include __DIR__ . '/prod.settings.php';  // Production environment configuration
}

$settings['trusted_host_patterns'] = [  
  '^tekbond\.com\.india$', 
  '^tekbond\.com\/india$',
  '^www\.tekbond\.com\/india$',
  '^localhost$',           
];



// Detect Acquia environment or custom environment variables
if (isset($_ENV['AH_SITE_ENVIRONMENT'])) {
  switch ($_ENV['AH_SITE_ENVIRONMENT']) {
    case 'local':
      $config['config_split.config_split.split_local']['status'] = TRUE;
      $config['config_split.config_split.split_staging']['status'] = FALSE;
      $config['config_split.config_split.split_prod']['status'] = FALSE;
      break;

    case 'staging':
      $config['config_split.config_split.split_local']['status'] = FALSE;
      $config['config_split.config_split.split_staging']['status'] = TRUE;
      $config['config_split.config_split.split_prod']['status'] = FALSE;
      break;
    ...
    case 'live':
      $config['config_split.config_split.split_local']['status'] = FALSE;
      $config['config_split.config_split.split_staging']['status'] = FALSE;
      $config['config_split.config_split.split_prod']['status'] = TRUE;
      break;
  }
} else {
  // Default to local environment if no environment variable is set
  $config['config_split.config_split.split_local']['status'] = TRUE;
  $config['config_split.config_split.split_staging']['status'] = FALSE;
  $config['config_split.config_split.split_prod']['status'] = FALSE;
}
